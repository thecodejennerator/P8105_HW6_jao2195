...
**Problem 3** **Problem 3A** Load and clean the data for regression
analysis <br> (i.e. convert numeric to factor where appropriate, check
...
``` r
#Plot of Fitted regressed on Residuals 
birthweight_cleaned|>
...
**Cross Validation** Make this comparison in terms of the
cross-validated prediction error; use `crossv_mc` and functions in purrr
as appropriate. Note that although we expect your model to be
reasonable, model building itself is not a main idea of the course and
we don’t necessarily expect your model to be “optimal”.

``` r
#Creating Cross Validation for Models 
cv_df = 
...
# Producing Final Corss-Validated Models 
